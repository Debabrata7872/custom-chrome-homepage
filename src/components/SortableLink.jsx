...
const SortableLink = ({ link, onEdit, onDelete, isLocked }) => {
  const { attributes, listeners, setNodeRef, transform, transition } = useSortable({ id: link.id, disabled: isLocked });
  ...
  return (
    <div ref={setNodeRef} style={style} {...(isLocked ? {} : attributes)} {...(isLocked ? {} : listeners)} className="relative group flex flex-col items-center w-14 sm:w-16 md:w-20 touch-none z-10 hover:z-50">
      <a href={link.url} className={`flex flex-col items-center gap-2 hover:-translate-y-2 transition-all duration-200 w-full ${isLocked ? 'cursor-pointer' : 'cursor-grab active:cursor-grabbing'}`}>
